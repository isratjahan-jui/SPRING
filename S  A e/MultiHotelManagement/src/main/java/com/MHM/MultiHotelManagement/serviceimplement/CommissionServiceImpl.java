...
public class CommissionServiceImpl implements CommissionService {
    ...
    private static final BigDecimal DEFAULT_RATE = BigDecimal.valueOf(5.0);
    ...
    public CommissionResponseDTO createFromBooking(CommissionRequestDTO dto) {
        ...
        BigDecimal baseAmount = dto.getPaymentAmount() != null ? dto.getPaymentAmount() : booking.getTotalPrice();
        BigDecimal adminEarningsBd = baseAmount.multiply(rate).divide(BigDecimal.valueOf(100), 2, java.math.RoundingMode.HALF_UP);
        BigDecimal ownerEarningsBd = baseAmount.subtract(adminEarningsBd);

        Commission commission = new Commission();
        commission.setBooking(booking);
        commission.setCommissionRate(rate);
        commission.setPaymentAmount(baseAmount);
        commission.setAdminEarnings(adminEarningsBd);
        commission.setHotelOwnerEarnings(ownerEarningsBd);
        commission.setCommissionStatus("ACTIVE");

        if (dto.getPaymentId() != null) {
            Payment payment = paymentRepo.findById(dto.getPaymentId()).orElse(null);
    ...
    @Override
    @Transactional
    public CommissionResponseDTO createForPayment(Payment payment, BigDecimal commissionRate) {
        ...
        if (commissionRepo.existsByPayment_Id(payment.getId())) {
            return null;
        }

        BigDecimal rate = commissionRate != null ? commissionRate : DEFAULT_RATE;
        BigDecimal payAmount = payment.getAmount();
        BigDecimal adminEarningsBd = payAmount.multiply(rate).divide(BigDecimal.valueOf(100), 2, java.math.RoundingMode.HALF_UP);
        BigDecimal ownerEarningsBd = payAmount.subtract(adminEarningsBd);

        Commission commission = new Commission();
        commission.setBooking(booking);
        commission.setPayment(payment);
        commission.setCommissionRate(rate);
        commission.setPaymentAmount(payAmount);
        commission.setAdminEarnings(adminEarningsBd);
        commission.setHotelOwnerEarnings(ownerEarningsBd);
        commission.setCommissionStatus("ACTIVE");
