...
export class BookHotel implements OnInit {
  ...
  foodItems: FoodItem[] = [];
  selectedFoodItemIds: Set<number> = new Set();
  ...
  private initPaymentOption() {
    ...
    if (this.hotel?.paymentType === 'CASH_ON_STAY') {
      this.advanceAmount = 0;
    } else if (this.hotel?.paymentType === 'PARTIAL_ADVANCE') {
      const pct = this.hotel.advancePercentage || 50;
      this.advanceAmount = Math.round((this.estimatedTotal * pct) / 100);
    } else {
      this.advanceAmount = this.estimatedTotal;
    }
  }
  ...
  get foodItemsTotal(): number {
    let total = 0;
    this.foodItems.forEach((f) => {
      if (this.selectedFoodItemIds.has(f.id)) {
        total += f.foodPrice;
      }
    });
    return total;
  ...
  get estimatedTotal(): number {
    return this.roomSubtotal - this.discountAmount + this.extraServicesTotal + this.foodItemsTotal;
  }
  ...
  submit() {
    ...
    const request: any = {
      ...
      // Deal-driven rate only — the coupon is applied and redeemed server-side
      // via couponCode so it isn't discounted twice.
      discountRate: this.discountRate,
      advanceAmount: 0,
    };
    if (this.couponApplied && this.couponCode.trim()) {
      request.couponCode = this.couponCode.trim();
    }
    if (this.selectedExtraServiceIds.size > 0) {
      request.extraServiceIds = Array.from(this.selectedExtraServiceIds);
    }
    if (this.selectedFoodItemIds.size > 0) {
      request.foodItemIds = Array.from(this.selectedFoodItemIds);
    }
  ...
  toggleFoodItem(id: number) {
    if (this.selectedFoodItemIds.has(id)) {
      this.selectedFoodItemIds.delete(id);
    } else {
      this.selectedFoodItemIds.add(id);
    }
  }
