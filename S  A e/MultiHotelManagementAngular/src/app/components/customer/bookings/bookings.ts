...
export class CustomerBookings implements OnInit {
  ...
  confirmCancel() {
    ...
    // Customer-allowed endpoint that also processes the wallet refund server-side.
    // (PATCH /status is admin/owner-only and would 403 here with no refund.)
    this.bookingService.cancelBooking(this.cancelTargetId).subscribe({
      next: () => {
  ...
  requestCheckIn(id: number) {
    ...
    // Customer-allowed online check-in (ID image optional here; can be uploaded
    // on the dedicated check-in page). PATCH /status is admin/owner-only.
    this.bookingService.onlineCheckIn(id).subscribe({
      next: () => {
  ...
  requestCheckOut(id: number) {
    if (
      ...
      return;
    this.cancellingId = id;
    // Customer-allowed express check-out. PATCH /status is admin/owner-only.
    this.bookingService.expressCheckOut(id).subscribe({
      next: () => {
        const b = this.bookings.find((x) => x.id === id);
