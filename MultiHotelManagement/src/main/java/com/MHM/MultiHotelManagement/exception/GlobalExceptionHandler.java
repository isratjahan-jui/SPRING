...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<Map<String, Object>> handleNoResource(
        ...
        // Browser probes (favicon) don't need an error body
        if (ex.getResourcePath() != null && ex.getResourcePath().contains("favicon")) {
            return ResponseEntity.noContent().build();
        }
        // Return via ResponseEntity so Spring's configured Jackson mapper (with the
        // Java-8 time module) serializes the LocalDateTime timestamp correctly —
        // a raw new ObjectMapper() cannot, which previously threw on every missing file.
        Map<String, Object> body = new LinkedHashMap<>();
        body.put("timestamp", LocalDateTime.now());
        body.put("status", HttpStatus.NOT_FOUND.value());
        body.put("error", "Not Found");
