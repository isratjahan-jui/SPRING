...
public class PaymentServiceImpl implements PaymentService {
    ...
    public PaymentResponseDTO createPayment(PaymentRequestDTO dto) {
        ...
        if (saved.getStatus() == PaymentStatus.PAID) {
            ...
            try {
                commissionService.createForPayment(saved, null);
                log.info("Commission created for payment {}", saved.getId());
                auditTrailService.logAction("COMMISSION_CREATED", "Commission", saved.getId(),
                        "Commission auto-created for payment " + saved.getId(), "SYSTEM");
            } catch (Exception e) {
                log.error("Commission creation failed for payment {}: {}", saved.getId(), e.getMessage(), e);
            }
        }
    ...
    public PaymentResponseDTO processRefund(Long bookingId) {
        ...
        if (payment.getStatus() == PaymentStatus.REFUNDED) {
            throw new IllegalStateException("Payment already refunded for booking #" + bookingId);
        }

        Booking booking = payment.getBooking();

        // If the booking was already cancelled, cancelBooking() already applied the
        // hotel's refund policy (which may legitimately be a zero refund). Refunding
        // the full amount here would pay out money the policy denied.
        if (booking.getStatus() == BookingStatus.CANCELLED) {
            throw new IllegalStateException(
                    "Booking #" + bookingId + " is already cancelled; refund was handled per policy at cancellation.");
        }
        ...
        // Credit the refund to the customer's wallet — this is the actual money movement
        // the notification below promises; it must happen before that message goes out.
        if (booking.getCustomer() != null && booking.getCustomer().getUser() != null
                && saved.getAmount() != null && saved.getAmount().compareTo(BigDecimal.ZERO) > 0) {
