...
public class AuthService {
    ...
    public LoginResponseDTO login(LoginRequestDTO dto) {
        ...
        // A.5: same message for unknown email and wrong password — a distinct
        // "not registered" response lets attackers enumerate valid accounts.
        if (user == null) {
    ...
    public void resetPassword(ResetPasswordRequestDTO dto) {
        ...
        // Single-use enforcement: a token issued before the last password change
        // (i.e. one that was already used, or predates a newer reset) is rejected.
        if (user.getPasswordChangedAt() != null) {
            LocalDateTime issuedAt = jwtUtil.extractIssuedAt(dto.getToken()).toInstant()
                    .atZone(java.time.ZoneId.systemDefault()).toLocalDateTime();
            if (!issuedAt.isAfter(user.getPasswordChangedAt())) {
                throw new BadRequestException("This reset link has already been used or is no longer valid");
            }
        }
