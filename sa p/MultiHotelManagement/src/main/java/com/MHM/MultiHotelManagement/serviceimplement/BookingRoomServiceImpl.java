...
public class BookingRoomServiceImpl implements BookingRoomService {
    ...
    public BookingRoomResponseDTO create(
        ...
        // Room আছে কিনা check — locked so a concurrent add-room call can't read
        // the same stale availableRooms count and both succeed.
    ...
    public BookingRoomResponseDTO getById(Long id) {
        ...
        ownershipGuard.verifyBookingAccess(bookingRoom.getBooking());
    ...
    public BookingRoomResponseDTO update(
        ...
        ownershipGuard.verifyHotelOwnership(bookingRoom.getBooking().getHotel());
    ...
    public void delete(Long id) {
        ...
        // Room availability restore করো — but only while the booking is still
        // holding the room; a terminal booking already restored its counters.
        BookingStatus parentStatus = bookingRoom.getBooking().getStatus();
        boolean bookingStillHoldsRoom = parentStatus == BookingStatus.PENDING
                || parentStatus == BookingStatus.CONFIRMED
                || parentStatus == BookingStatus.CHECKED_IN;
        if (bookingStillHoldsRoom) {
            Room room = bookingRoom.getRoom();
            room.setAvailableRooms(
    ...
    public void deleteAllByBookingId(Long bookingId) {
        ...
        // Only restore availability while the booking still holds the rooms —
        // a terminal booking already restored its counters.
        BookingStatus parentStatus = booking.getStatus();
        boolean bookingStillHoldsRoom = parentStatus == BookingStatus.PENDING
                || parentStatus == BookingStatus.CONFIRMED
                || parentStatus == BookingStatus.CHECKED_IN;
        ...
        bookingRooms.forEach(br -> {
            ...
            if (room != null && bookingStillHoldsRoom) {
                room.setAvailableRooms(
                        room.getAvailableRooms()
                                + br.getNumberOfRooms()
                );
